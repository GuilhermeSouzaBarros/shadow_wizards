...
class Shape:
    ...
    def delta_position(self, delta_time):
        self.position.x += (self.speed.x * delta_time)
        self.position.y += (self.speed.y * delta_time)
    
    def next_position(self, delta_time):
        copy = Vector2(self.position.x, self.position.y)
        copy.x += self.speed.x * delta_time
        copy.y += self.speed.y * delta_time
        return copy
...
class Circle(Shape):
    ...
    def collision_line(self, line, delta_time):
